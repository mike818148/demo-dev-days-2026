...
import { getOrganizations } from "@/lib/actions/nerm";
import type { NermProfile } from "@/lib/actions/nerm";
import { Building2, RefreshCw, Eye } from "lucide-react";
...
import { NermOrganizationDetails } from "./nerm-organization-details";


export function NermOrganization() {
      ...
      <div className="flex flex-col gap-4">
        ...
        <Empty className="border border-dashed">
          <EmptyHeader>
            ...
            <EmptyDescription>
              No organizations found. Create one from New Organization.
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
      </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">{data.length} organization(s)</p>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
      ...
      <Dialog open={detailsProfile !== null} onOpenChange={(open) => !open && setDetailsProfile(null)}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto border-border/80 shadow-xl">
          {detailsProfile && <NermOrganizationDetails profile={detailsProfile} />}
        </DialogContent>
      </Dialog>
    </div>
  );
